...
export async function createCommunity(
    ...
    image:string,
    bio:string,
    createdById:string, //change the parameter name to reflect its an id
){
  try {
    connectToDB();
    ...
    if(!user){
        throw new Error("User not Found")// Handle the case if the user
        // with the id is not found
    }
...
export async function addMemberToCommunity(communityId: string, memberId: string){
  try {
    ...
    // check if the user is alread a member of the community
...
interface PropsupdateCommunityInfo{
    communityId: string;
    name: string;
    username: string;
    image: string;
...
export async function deleteCommunity(communityId: string){
  try {
    ...
    // Find the users who are part of the community
    const communityUsers = await User.find({communities: communityId});

    // Remove the community form the communities array of each user
    const updatedUserPromies = communityUsers.map((user) => {
        user.communities.pull(communityId)

        return user.save()
    });

    await Promise.all(updatedUserPromies);

...
interface Props {
  ...
  id: string;
    like: boolean;
    likecount: number; // Assuming likeCount is a number, update it accordingly
    setLike: Dispatch<SetStateAction<boolean>>;
    setLikeCount: Dispatch<SetStateAction<number>>;
}

const LoadingStateImage = ({ image, currentUserId, id, like, likecount, setLike, setLikeCount }: Props) => {
  ...
  return (
    <div className='relative w-full h-full '>
      ...
      <div onDoubleClick={likeThreads} className='absolute bg-[rgba(255,255,255,0.01)] z-20 flex justify-center items-center  w-full h-full top-0 left-0'>
        ...
        {bigHeart && (
          <svg width="200px" height="200px" className={`${popAnimation && 'likeButton'} cursor-pointer`}
